...
import { Camera, ScreenShare, Layers } from "lucide-react"
import { Button } from "@/components/ui/button"
import { Dialog, DialogContent, DialogDescription, DialogFooter, DialogHeader, DialogTitle } from "@/components/ui/dialog"
import { uploadCreatorVideo } from "@/lib/uploadthing/client"
...
export default function CreatorLivePage() {
  ...
  return (
    <div className="relative min-h-[calc(100dvh-100px)] bg-black text-white">
      <div className="absolute inset-0 overflow-hidden">
        <video
          ref={localVideoRef}
          autoPlay
        ...
        <div className="absolute inset-x-0 top-0 p-4">
          <div className="flex flex-col gap-2 max-w-5xl mx-auto">
              ...
              <div>
                ...
                {sourceMode === "screen" || sourceMode === "both" ? (
                  <div className="flex items-center gaps-2">
                    ...
                    <p className="text-sm hidden md:block text-amber-200">Screen recording mode</p>
                  </div>
                ) : null}
                {recordingUrl ? (
                  <p className="text-sm text-emerald-300">Recording uploaded</p>
            ...
            <div className="flex flex-wrap gap-2">
              ...
              {/* Screen-share dependent buttons are hidden entirely when unsupported */}
              ...
              {/* {screenShareSupported ? (
                ...
                >
                  ...
                  <span className="hidden sm:inline">Camera + screen share</span>
                </button>
              ) : null} */}
            </div>
            {cameras.length > 0 ? (
              <div className="flex items-center gap-2">
                <Camera className="h-4 w-4 text-slate-300 shrink-0 sm:hidden" />
                <select
                  value={selectedCamera ?? ""}
                  onChange={(e) => handleSwitchCamera(e.target.value)}
                  disabled={paused}
                  className={`w-full max-w-[220px] rounded-full border border-white/30 bg-black/40 px-3 py-2 text-sm text-slate-200 transition focus:outline-none focus:border-white ${
                    paused ? "cursor-not-allowed opacity-50" : ""
                  }`}
                >
                  {cameras.map((camera) => (
