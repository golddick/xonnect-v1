...
export default function CreatorLivePage() {
  ...
  const [cameras, setCameras] = useState<Array<{ deviceId: string; label: string }>>([])
  const [selectedCamera, setSelectedCamera] = useState<string | null>(null)
  // FIX: mobile camera switching now goes through facingMode, not deviceId.
  // deviceId-based getUserMedia calls are unreliable on phones — most
  // browsers just hand back whatever camera is already active instead of
  // actually switching. facingMode is the constraint mobile browsers honor.
  ...
  const [isIOS, setIsIOS] = useState(false)
  const [isSafari, setIsSafari] = useState(false)
  // FIX: drives whether we show the mobile "flip camera" button or the
  // desktop deviceId dropdown.
  ...
  useEffect(() => {
    ...
    setIsIOS(/iphone|ipad|ipod/.test(userAgent))
    setIsSafari(/safari/.test(userAgent) && !/chrome/.test(userAgent))
    setIsMobileDevice(/android|iphone|ipad|ipod/.test(userAgent))
  }, [])
  ...
  // Only load the deviceId list on desktop — it's used to populate the
  // dropdown there. Mobile uses facingMode instead, so this is skipped.
  useEffect(() => {
  ...
  // Pre-live preview: mirror the currently selected camera into the preview
  // <video> before going live, so the creator sees exactly which camera they
  // picked. On mobile this follows facingMode; on desktop it follows the
  // deviceId dropdown. Once connected, the published LiveKit track drives
  // the preview and this effect stands down (releasing its getUserMedia
  // stream).
  useEffect(() => {
    const stopPreview = () => {
      if (previewStreamRef.current) {
    ...
    if (connected || sourceMode === "screen") {
      stopPreview()
      return
    }

    if (!isMobileDevice && !selectedCamera) {
      stopPreview()
      return
    }
    ...
    ;(async () => {
      try {
        ...
        // A busy, over-constrained, or nonexistent (e.g. no rear camera)
        // device just means no preview; not fatal. Retry mobile without
        // "exact" so a device with only one camera doesn't hard-fail.
        if (isMobileDevice) {
          try {
            ...
            stopPreview()
            previewStreamRef.current = fallbackStream
            if (localVideoRef.current) {
              localVideoRef.current.srcObject = fallbackStream
            }
            return
          } catch (fallbackErr) {
            console.warn("Camera preview failed", fallbackErr)
          }
        } else {
          console.warn("Camera preview failed", err)
        }
  ...
  }, [selectedCamera, connected, sourceMode, isMobileDevice, facingMode])
  ...
  // FIX: accepts either a deviceId (desktop) or a facingMode (mobile) so both
  // the initial publish and the switch/flip paths can share one function.
  const publishCameraTrack = async (
    room: Room,
    opts: { deviceId?: string | null; facingMode?: "user" | "environment" }
  ) => {
    // Release the pre-live preview stream first so the same camera device isn't
    // held twice when we open it for the published track.
    if (previewStreamRef.current) {
    ...
    const videoConstraints: VideoCaptureOptions = opts.facingMode
    ...
    let vTrack
    try {
      vTrack = await createLocalVideoTrack(videoConstraints)
    } catch (err) {
      // Mobile fallback: if "exact" facingMode fails (e.g. device only has
      // one camera), retry with a loose facingMode hint instead of failing.
      if (opts.facingMode) {
        vTrack = await createLocalVideoTrack({ facingMode: opts.facingMode })
      } else {
        throw err
      }
    }
  ...
  // Helper: builds the right constraint object for "whichever camera source
  // is currently selected" — deviceId on desktop, facingMode on mobile.
  const currentCameraOpts = () =>
    isMobileDevice ? { facingMode } : { deviceId: selectedCamera }
    ...
    try {
      ...
      if (nextMode === "camera") {
        if (publishedScreenRef.current) {
          ...
          if (!publishedVideoRef.current) {
            const vTrack = await publishCameraTrack(room, currentCameraOpts())
            try {
              vTrack.attach(localVideoRef.current!)
            } catch {}
          }
  ...
  const handleConnectAndPublish = async () => {
    ...
    try {
      ...
      if (sourceMode === "camera" || sourceMode === "both") {
        primaryVideoTrack = await publishCameraTrack(r, currentCameraOpts())
      }
  ...
  // Desktop path: switch camera by deviceId from the dropdown.
  const handleSwitchCamera = async (deviceId: string) => {
    if (!deviceId || publishing) return
    setSelectedCamera(deviceId)
    ...
    try {
      ...
      // FIX: release the old track BEFORE opening the new one. Holding two
      // camera sessions open at once is unreliable on many devices and can
      // cause the "new" open to just hand back the same camera.
      if (oldTrack) {
        try {
          await room.localParticipant.unpublishTrack(oldTrack)
        } catch (err) {
  ...
  // FIX: mobile path. Flips between front ("user") and back ("environment")
  // camera using facingMode instead of deviceId — the constraint mobile
  // browsers actually respect. Always releases the currently published
  // camera track before requesting the new one, since phones generally
  // can't hold two camera sessions open at the same time.
  const handleFlipCamera = async () => {
    if (publishing) return
    const nextFacing: "user" | "environment" = facingMode === "user" ? "environment" : "user"

    // Not live yet (or paused): just flip the preview.
    if (!room || !connected || paused) {
      setFacingMode(nextFacing)
      return
    }

    // Screen-only stream has no published camera track to swap; keep the
    // selection so it applies if the creator switches back to camera mode.
    if (sourceMode === "screen") {
      setFacingMode(nextFacing)
      return
    }
    ...
    try {
      ...
      // Note: LiveKit's createLocalVideoTrack only accepts a plain
      // facingMode string ("user" | "environment" | "left" | "right"), not
      // a native { exact: ... } constraint object — that's TS/browser-API
      // territory, not LiveKit's.
      const newTrack = await createLocalVideoTrack({ facingMode: nextFacing })
      ...
      <div className="absolute inset-0 overflow-hidden">
        ...
        <div className="absolute inset-x-0 top-0 p-4">
          <div className="flex flex-col gap-2 w-full">
            ...
            {/* FIX: mobile gets a simple flip button (facingMode-based);
                desktop keeps the deviceId dropdown. Both are hidden while
                in screen-only mode since there's no camera track to switch. */}
            {sourceMode !== "screen" && isMobileDevice ? (
              <button
                type="button"
                onClick={handleFlipCamera}
                disabled={publishing || paused}
                aria-label="Flip camera"
                className={`flex w-fit items-center gap-2 rounded-full border border-white/30 bg-black/40 px-3 py-2 text-sm text-white transition ${
                  publishing || paused ? "cursor-not-allowed opacity-50" : "cursor-pointer"
                }`}
              >
            ...
            ) : sourceMode !== "screen" && cameras.length > 0 ? (
              <div className="flex items-center gap-2">
                <Camera className="h-4 w-4 shrink-0 text-slate-300" />
                <select
                  value={selectedCamera ?? ""}
