...
export default function UploadVideoComponent({ onClose, onUpload, initialFolderId = null }: UploadVideoProps) {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        if (initialFolderId && !initialFolderSet.current) {
          const matchingFolder = fetchedFolders.find((f: any) => f.id === initialFolderId)
          if (matchingFolder) {
            setFormData((prev) => ({
              ...prev,
              folderId: matchingFolder.id,
              contentType: matchingFolder.folderType ?? prev.contentType,
            }))
  ...
  // Handle initial folder selection after folders are loaded
  useEffect(() => {
    if (folders.length > 0 && initialFolderId && !initialFolderSet.current) {
      const matchingFolder = folders.find((f) => f.id === initialFolderId)
      if (matchingFolder) {
        setFormData((prev) => ({
          ...prev,
          folderId: matchingFolder.id,
        ...
        initialFolderSet.current = true
        console.log("Auto-selected folder:", matchingFolder.id, matchingFolder.title)
      }
    }
  ...
  if (file.size > 5 * 1024 * 1024) {
    setThumbnailError("Image must be less than 5MB")
    return
  }
  ...
  try {
    ...
    setFormData((prev) => ({
      ...
      thumbnailFileId: result.fileId ?? null,
    }))
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Prepare thumbnail data (already uploaded via DropAphi)
      const thumbnailUrl = formData.thumbnailUrl
      const thumbnailFileId = formData.thumbnailFileId
      ...
      if (formData.contentType === "series") {
        ...
        for (let i = 0; i < formData.episodes.length; i++) {
          ...
          const episodePayload = {
            ...
            packageName: formData.packageName || formData.contentType,
            episodeIndex: i,
            videoUrl: ep.videoUrl,
            videoFileId: ep.videoFileId,
            duration: ep.duration ?? null,
                  ...
                  {formData.folderId && (
                    <div className="flex items-center gap-2 px-3 py-1 bg-green-600/10 border border-green-600/20 rounded-full">
                      <span className="text-xs text-green-600 font-medium">
                        {folders.find(f => f.id === formData.folderId)?.title || "Folder selected"}
                      </span>
                    </div>
                  )}
                </div>
                    ...
                    {formData.folderId && (
                      <div className="flex items-center gap-2 p-3 bg-green-600/10 border border-green-600/20 rounded-lg">
                        ...
                        <span className="text-sm text-green-600">
                          Folder selected: {folders.find(f => f.id === formData.folderId)?.title}
                          {initialFolderId === formData.folderId && " (auto-selected)"}
                        </span>
                      </div>
                    )}
                  </div>
          ...
          {currentStep === 2 && (
            <div className="space-y-6">
              <div className="text-center">
                ...
                <p className="text-muted-foreground">
                  {formData.contentType === "series"
                    ? `Upload the episodes for "${formData.packageName || 'Series'}"`
                    : `Select the video file for your ${formData.contentType}`}
                </p>
              </div>
              ...
              {formData.contentType === "series" ? (
                <div className="space-y-4">
                    ...
                    {formData.episodes.map((ep, index) => (
                      <div key={ep.id} className="bg-muted/50 border border-border rounded-xl p-4 flex items-center justify-between">
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            <p className="font-medium text-foreground">{ep.title || `Episode ${index + 1}`}</p>
                            {ep.videoFileId ? (
                  ...
                  <div className="border-2 border-dashed border-border rounded-2xl p-8 text-center hover:border-muted-foreground/50 transition-colors">
                    <UploadButton
                      ...
                      onClientUploadComplete={(res) => {
                        ...
                        if (!videoUrl || !videoFileId) {
                          alert("Failed to get video URL from upload")
                          return
                        }
                    ...
                    />
                    <p className="text-sm text-muted-foreground mt-2">Upload video files (MP4, MOV, etc.)</p>
                  </div>
                </div>
              ) : (
                <div className="space-y-6">
                  ...
                  {!videoUploadComplete && (
                    <div className="border-2 border-dashed border-border rounded-2xl p-12 text-center hover:border-muted-foreground/50 transition-colors">
                      <UploadButton
                        ...
                        onClientUploadComplete={(res) => {
                          const uploaded = res?.[0] as any
                          console.log("UploadThing complete (single):", res, "uploaded:", uploaded)
                      ...
                      />
                      <p className="text-sm text-muted-foreground mt-2">Upload video file (MP4, MOV, etc.)</p>
                    </div>
                  )}
                </div>
              )}
                ...
                <div className="bg-muted/50 border border-border rounded-xl p-4">
                  <div className="flex items-center gap-4">
                    ...
                    <div className="flex-1">
                      <h4 className="font-semibold text-foreground">Video uploaded</h4>
                      <p className="text-muted-foreground text-sm">
                        {formData.videoFileId ? `File ID: ${formData.videoFileId.substring(0, 12)}...` : "Ready"}
                      </p>
                      {formData.videoDuration && (
                        <p className="text-xs text-muted-foreground">Duration: {formData.videoDuration}</p>
                      )}
                    </div>
                    <button
                      type="button"
                  ...
                  <div>
                    <div className="border-2 border-dashed border-border rounded-xl p-6 text-center hover:border-muted-foreground/50 transition-colors">
                      ...
                      <label htmlFor="thumbnail-upload" className="cursor-pointer block">
                        <ImageIcon className="w-8 h-8 text-muted-foreground mx-auto mb-2" />
                        <p className="text-muted-foreground text-sm">
                          {thumbnailUploading ? "Uploading..." : "Upload custom thumbnail"}
                        </p>
                        <p className="text-muted-foreground/60 text-xs mt-1">PNG, JPG up to 5MB</p>
                      </label>
                    </div>
                    ...
                    {formData.thumbnailUrl && (
                      <div className="mt-2 p-2 bg-green-600/10 border border-green-600/20 rounded-lg">
                        <p className="text-xs text-green-600">✓ Thumbnail uploaded to DropAphi</p>
                      </div>
                    )}
                  </div>
