...
export async function PUT(
  ...
  try {
    ...
    const updated = await db.$transaction(async (tx) => {
      const eventUpdate = await tx.creatorEvent.update({
        ...
        data: {
          ...
          hasRecordedVideo: body.hasRecordedVideo ?? event.hasRecordedVideo,
    ...
    // If status changed to LIVE (and wasn't already LIVE), notify followers
    if (event.status !== "LIVE" && updated.status === "LIVE") {
      try {
