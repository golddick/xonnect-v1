  ...
  try {
    ...
    if (!Number.isFinite(amount) || amount <= 0 || amount > availableForPayout || amount < minimumPayoutAmount) {
      return NextResponse.json(
        { message: `Requested amount is not allowed. Minimum payout amount is ₦${minimumPayoutAmount}.` },
        { status: 400 }
      )
    }
    ...
    return NextResponse.json({ payoutRequest, message: "Payout request created" }, { status: 201 })
  } catch (error) {
    console.error("Creator payout verify error:", error)
    return NextResponse.json({ message: "Failed to verify payout request" }, { status: 500 })
  }
