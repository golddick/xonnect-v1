...
function mapCheckInUser(user: any): CheckInSessionUser {
  return {
    id: user.id,
    email: user.email,
...
export async function getAuthenticatedCheckInUser(request: Request) {
  ...
  return mapCheckInUser(user)
}

/**
 * Resolve a check-in operator from an active camera-session token. Lets a nearby
 * phone (which holds the camera token in its URL but has no session cookie) act
 * as the operator that generated the session.
 */
export async function getCheckInUserFromCameraToken(token: string) {
  if (!token) return null
  ...
  if (!isCameraStatusActive(status)) return null
  if (session.expiresAt && session.expiresAt.getTime() <= Date.now()) return null

  const user = session.operatorUser
...
/**
 * Authenticate a scan/lookup request via the operator session cookie, falling
 * back to a camera-session token supplied by a paired phone.
 */
export async function getScanActor(request: Request, cameraToken?: string | null) {
  const cookieUser = await getAuthenticatedCheckInUser(request)
  if (cookieUser) return cookieUser

  if (cameraToken) {
    return getCheckInUserFromCameraToken(cameraToken)
  }
...
/**
 * Look up a VENUE ticket for an event by scanned code (or purchase id fallback).
 * Returns the matching ticket item (for multi-quantity purchases) and/or the
 * parent purchase. Shared by the scan (mutating) and lookup (read-only) routes.
 */
export async function findVenueTicketByCode(eventId: string, parsed: ParsedCheckInPayload) {
...
export function parseCheckInPayload(code: string): ParsedCheckInPayload {
